...
static void	get_64_header_and_section(void *file_map, Elf64_Ehdr **header64, Elf64_Shdr **sections_header64)
{
	*header64 = (Elf64_Ehdr *)file_map;
	*sections_header64 = (Elf64_Shdr *)(file_map + (*header64)->e_shoff);
}

static void	get_32_header_and_section(void *file_map, Elf32_Ehdr **header32, Elf32_Shdr **sections_header32)
{
...
bool	save_symbols_64(void *file_map, t_symbol_entry **symbols, size_t *symbol_count)
{
	...
	*symbol_count = get_symbols_count_64(file_map, sections_header, sections_count);
...
bool	save_symbols_32(void *file_map, t_symbol_entry **symbols, size_t *symbol_count)
{
	Elf32_Ehdr		*header;
	...
	*symbol_count = get_symbols_count_32(file_map, sections_header, sections_count);
	*symbols = (t_symbol_entry *)malloc(sizeof(t_symbol_entry) * *symbol_count);
	if (!(*symbols))
	{
		perror("malloc");
...
bool	save_symbols(void *file_map, t_symbol_entry **symbols, size_t *symbol_count)
{
	if (is_64)
	{
		if (save_symbols_64(file_map, symbols, symbol_count))
			return true; // error
	}
	else
	{
		if (save_symbols_32(file_map, symbols, symbol_count))
			return true; // error
	}
	return false;
}