					 ...
					 size_t file_size)
{
	...
	*symbol_count =
		get_symbols_count_64(file_map, sections_header, sections_count, file_size);
					 ...
					 size_t file_size)
{
	...
	*symbol_count =
		get_symbols_count_32(file_map, sections_header, sections_count, file_size);
	if (*symbol_count == 0 || *symbol_count == -1)
	{
		if (*symbol_count == -1)
			return true;
				  ...
				  size_t file_size)
{
	if (is_64)
	{
		if (save_symbols_64(file_map, symbols, symbol_count, file_name, file_size))
			return true; // error
	}
	else
	{
		if (save_symbols_32(file_map, symbols, symbol_count, file_name, file_size))
			return true; // error
	}
	return false;
}