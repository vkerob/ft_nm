							...
							size_t *pos_symbols)
{
	...
	Elf32_Sym *symtab
		= (Elf32_Sym *)((char *)file_map + symtab_section->sh_offset);
							...
							size_t *pos_symbols)
{
	// keep variables for the symbol table and string table
	Elf64_Sym *symtab
		= (Elf64_Sym *)((char *)file_map + symtab_section->sh_offset);
	const char *strtab = (const char *)file_map + strtab_section->sh_offset;
					 ...
					 t_symbol_entry *symbols, Elf32_Half sections_count)
{
	...
	for (int i = 0; i < sections_count; i++)
	{
		if (sections[i].sh_type == SHT_SYMTAB)
		{
			...
			set_target_symbols_32 (file_map, sections, symtab_section,
					 ...
					 t_symbol_entry *symbols, Elf64_Half sections_count)
{
	...
	for (int i = 0; i < sections_count; i++)
	{
		if (sections[i].sh_type == SHT_SYMTAB)
		{
			symtab_section = &sections[i];
			strtab_section = &sections[symtab_section->sh_link];
			set_target_symbols_64 (file_map, sections, symtab_section,
								   strtab_section, symbols, &pos_symbols);
			break;
		}
	}
